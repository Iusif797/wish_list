    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          ...
          <Link
            href="/dashboard/new"
            className="px-5 py-2.5 bg-gradient-to-r from-primary-500 to-emerald-500 text-white font-semibold rounded-xl hover:from-primary-600 hover:to-emerald-600 transition-all shadow-glow hover:shadow-glow-lg flex items-center gap-2"
          >
        ...
        {error && (
          <div className="p-4 bg-red-50 border border-red-100 text-red-700 rounded-xl animate-scale-in">
            {error.message}
          </div>
        )}
        {wishlists && wishlists.length === 0 && (
          <div className="glass-card rounded-3xl p-16 text-center animate-slide-up">
            ...
            <p className="text-slate-600 mb-6 text-lg">You don&apos;t have any wishlists yet.</p>
            <Link
              href="/dashboard/new"
            ...
            {wishlists.map((w, i) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-50 to-emerald-50 flex items-center justify-center mb-3 group-hover:from-primary-100 group-hover:to-emerald-100 transition-colors">
                    <span className="text-lg">🎁</span>
                  </div>
                  <svg className="w-5 h-5 text-slate-300 group-hover:text-primary-500 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" /></svg>
                </div>
                <h2 className="font-bold text-slate-900 text-lg">{w.name}</h2>
                <p className="text-sm text-slate-500 mt-1">{w.occasion}</p>
                <div className="mt-3 inline-flex items-center gap-1.5 px-2.5 py-1 bg-primary-50 text-primary-700 text-xs font-medium rounded-lg">
